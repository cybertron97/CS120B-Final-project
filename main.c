/*
 *Custom Project made by : Tanish Arora 
 * Introduction:   
 *
 *The game of dodging, basically 
 *tests user’s reaction time to 
 *the upcoming custom obstacles, 
 *both rows of the LCD’s contains 
 *randomly occurring obstacles which 
 *come with the period and leaves. The Game is based on the 
 *original game of Dino in Google Chrome (the most awesome 
 *game which usually shows up when there’s no internet connection.
 *The difference between these two games is that in the T-Rex game, 
 *you just have to dodge the bottom obstacles by jumping, 
 *thus making it much easier for the user to score really high scores 
 *but for the dodge game, the user has two rows of obstacles thus increasing the chance 
 *of getting struck by the obstacle and losing the game within the first few secs of the game!
 *
 *
 *Helpful links : 
...
#define total_game_length 16   // althought the LCD is a 16x2 I still needed the game to run more to introduce new levels if possible 
#define b_one 7        //Bottom first obstacle position 
#define T_one 11       // Top row first obstacle postion
#define T_two 16       //Top row 2nd obstacle positon
...
unsigned short x,y;           // somehow defining it into the function didnt solve, have to define it globally. 
...
const unsigned char GameOver2[] = "    OVER :--(   "; // Bottom game over messsage
...
int gameTrack_tickfct(int state) {
	...
	switch(state) { // Transitions
		...
		case gameTrack_wait:if (game_OVER) {state= gameTrack_gameOver;}  //if game was lost, then charcater freezes. and reset to first position
...
int firstRow_tickfct(int state) {
	static unsigned char a, b;  //changing it to static to make it into active state as it changes with the game
		...
		case firstRow_initial: a = 1; //cursor positionat 1
...
int PlayerMovement_tickfct(int state) {static unsigned char a; //not really need another
	switch(state) { // Transitions
		...
		//move the charcater to the bottom row
